...
def all_plugins():
    plugins = {"UserAssist":{'function': UserAssist.UserAssist,"Target_hives":"NTUSER.DAT","Discription":"test"},
                ...
                "REGTimeLine":{'function':REGTimeLine.regtimeline,"Target_hives":"all","Discription":"Collect services from the SYSTEM registry"}
                }
...
def print_for_kuiper(file,log,plugin):
    if file is not None:
        ...
        defined_logs =[]
        plugins = all_plugins()
        plugin_sig_hive = plugins[plugin]['Target_hives']
        if plugin_sig_hive == "all":
            plugin_sig_hive = file.split("/")[-1]
        log_ext = [".LOG",".LOG1",".LOG2"]
        for lg in log:
            for ex in log_ext:
